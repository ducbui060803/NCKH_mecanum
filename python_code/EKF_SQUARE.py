...
class EKF:
    ...
    def update(self, z_t):
        ...
        # chi2_value = LowPassFilter(chi2_value)
        ...
        if chi2_value < self.chi2_threshold:
            ...
            print(f"[Get] χ² = {chi2_value:.2f}")

        else:
            print(f"[Warning] Bỏ qua đo lường do χ² = {chi2_value:.2f} vượt ngưỡng {self.chi2_threshold}")\
...
a_a = 0.1
chi = None
def LowPassFilter(chichi):
    global chi, a_a
    if chi is None:
        chi = chichi
    else:
        chi = a_a * chichi + (1 - a_a) * chi
    return chi
try:
    # Initialize ROS node
